...
async function runPipeline() {
    ...
    const agentDisplay = AGENT_DISPLAY[agentKey];
    const apiKey = CONFIG.API_KEYS[agentKey];

    resetPipeline();
    $('pipelineBadge').textContent = 'RUNNING';
    $('pipelineBadge').classList.add('running');

    // ── If no API key configured, show config prompt ──────────────
    if (!apiKey) {
        toast('No API key configured for this agent. Click the Settings button to add keys.', '⚠️');
        $('pipelineBadge').classList.remove('running');
        $('pipelineBadge').textContent = 'NO KEY';
        state.running = false;
        $('submitBtn').disabled = false;
        return;
    }

    // ── Phase 1 animation (client-side) ──────────────────────────
    animatePhaseScanning(1);
    await sleep(400);
    ...
    try {
        ...
        const res = await fetch(`${CONFIG.API_BASE}/v1/gateway`, {
            method: 'POST',
            headers: {
        ...
        toast(`Backend error: ${apiError}`, '⚠️');
        $('pipelineBadge').classList.remove('running');
    ...
    // ── Render phase animations based on API response ─────────────
    await renderPipelineFromResponse(apiResponse, httpStatus, agentDisplay, agentKey, prompt, context);
}
